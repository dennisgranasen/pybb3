...
Status: specification for the current semantic timeline and compact narrative
projection produced by `Replay.timeline()`. The schema remains additive and
extensible as new BB3 protocol mechanics are identified.
...
### TimelineCheck (narrative projection)

`checks` is generated by `to_narrative_dict()` / `to_narrative_json()`; it is
not a field on the full `TimelineEvent` dataclass returned by
`Replay.timeline().to_dict()`. It is the compact semantic projection of roll
evidence stored on the full event.
...
| `attempts[].reroll` | string or null | Reroll source for this attempt, for example `team`, `dodge`, or another skill-derived name. The first attempt omits it. |
...
| `reroll_used` | boolean | Whether a reroll represented by this check was actually used. Inspect `attempts[].reroll` for its source; do not infer `team` solely from availability. |
